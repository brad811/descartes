  ...
  return $.ajax({
    ...
    if (d.length === 0) {
      ...
      for (var i in d) {
        ...
        $('div.graphs div.row span#' + d[i].uuid).append('<a href="/graphs/' + d[i].uuid + (($.url().param('interval')!=null)?'?interval=' + $.url().param('interval'):'') + '"><img src="' + constructGraphUrl(c) + '" alt="' + d[i].name + '" name="' + d[i].uuid + '" /></a>');
